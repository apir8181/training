...
void init_value() {
    genRandomStr(value, MAXVALUESIZE/2);
}

void set_sprintf(char *buf) {
    sprintf(buf, "set %s %s", key[rand() % NTESTDATA].c_str(), value.c_str());
}
...
void test(unsigned short port, int count) {
    ...
    vector<TestThread *> threads = create_threads(port, count, set_sprintf);

    int time = -1;
    while (time < TESTTIME) {
	update_threads(port, count, set_sprintf, threads);
	sleep(1);
